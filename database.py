...
def init_db():
    with get_connection() as conn:
        with conn.cursor() as c:
            ...
            c.execute('''CREATE TABLE IF NOT EXISTS fault_codes (
                id SERIAL PRIMARY KEY,
                vehicle_id TEXT,
                truck_number TEXT,
                code TEXT,
            ...
            c.execute('''CREATE TABLE IF NOT EXISTS harsh_events (
                id SERIAL PRIMARY KEY,
                vehicle_id TEXT,
                truck_number TEXT,
                event_type TEXT,
            ...
            c.execute('''CREATE TABLE IF NOT EXISTS maintenance_alerts (
                id SERIAL PRIMARY KEY,
                vehicle_id TEXT,
                truck_number TEXT,
...
def save_fault_code(vehicle_id, truck_number, code, description, severity):
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute('''INSERT INTO fault_codes (vehicle_id, truck_number, code, description, severity)
                         VALUES (%s, %s, %s, %s, %s)''',
...
def get_all_fault_codes():
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute("SELECT vehicle_id, truck_number, code, description, severity, recorded_at FROM fault_codes ORDER BY recorded_at DESC")
            return c.fetchall()

# ==================== HARSH EVENTS ====================

def save_harsh_event(vehicle_id, truck_number, event_type, location, video_url):
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute('''INSERT INTO harsh_events (vehicle_id, truck_number, event_type, location, video_url)
                         VALUES (%s, %s, %s, %s, %s)''',
...
def get_all_harsh_events():
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute("SELECT vehicle_id, truck_number, event_type, location, video_url, happened_at FROM harsh_events ORDER BY happened_at DESC")
            return c.fetchall()

# ==================== MAINTENANCE ALERTS ====================

def save_maintenance_alert(vehicle_id, truck_number, maintenance_type, due_mileage, location):
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute('''INSERT INTO maintenance_alerts (vehicle_id, truck_number, maintenance_type, due_mileage, location)
                         VALUES (%s, %s, %s, %s, %s)''',
...
def get_all_maintenance_alerts():
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute("SELECT vehicle_id, truck_number, maintenance_type, due_mileage, location, created_at FROM maintenance_alerts ORDER BY created_at DESC")
            return c.fetchall()
...
def get_driver_by_truck(truck_number):
    with get_connection() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as c:
            ...
            r = c.fetchone()
            if r:
                return {"chat_id": r["telegram_chat_id"], "driver_id": r["samsara_driver_id"], "vehicle_id": r["vehicle_id"], "truck_number": r["truck_number"], "truck_license": r["truck_license"] or "", "password": r["password"] or ""}
            return None
...
def get_truck_stops_near(lat, lng, radius_miles=100, brands=None):
    ...
    with get_connection() as conn:
        with conn.cursor() as c:
            ...
            rows = c.fetchall()
    return [{"name": r[0], "brand": r[1], "address": r[2], "city": r[3], "state": r[4], "lat": r[5], "lng": r[6], "price": r[7]} for r in rows]
...
def add_points(driver_id, amount, type, description=""):
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute("INSERT INTO points (driver_id, amount, type, description) VALUES (%s, %s, %s, %s)", (driver_id, amount, type, description))
...
def get_points_history(driver_id, limit=20):
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute("SELECT amount, type, description, created_at FROM points WHERE driver_id = %s ORDER BY created_at DESC LIMIT %s", (driver_id, limit))
            return c.fetchall()
...
def add_pti_submission(driver_id, truck_number, trailer_number, photo_count):
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute("INSERT INTO pti_submissions (driver_id, truck_number, trailer_number, photo_count, status) VALUES (%s, %s, %s, %s, 'submitted')", (driver_id, truck_number, trailer_number, photo_count))
...
def get_pti_history(driver_id, limit=20):
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute("SELECT pti_number, submitted_at, status FROM pti_submissions WHERE driver_id = %s ORDER BY submitted_at DESC LIMIT %s", (driver_id, limit))
            return c.fetchall()
...
def add_fuel_station_usage(driver_id, station_name, station_address, station_lat, station_lng, price):
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute("INSERT INTO fuel_station_usage (driver_id, station_name, station_address, station_lat, station_lng, price) VALUES (%s, %s, %s, %s, %s, %s)", (driver_id, station_name, station_address, station_lat, station_lng, price))

def get_fuel_station_usage(driver_id, limit=20):
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute("SELECT station_name, station_address, station_lat, station_lng, price, used_at, is_confirmed FROM fuel_station_usage WHERE driver_id = %s ORDER BY used_at DESC LIMIT %s", (driver_id, limit))
            return c.fetchall()
...
def get_verification(telegram_user_id):
    with get_connection() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as c:
            c.execute("SELECT driver_name, truck_number, truck_photo_path, verified FROM driver_verification WHERE telegram_user_id = %s", (telegram_user_id,))
            r = c.fetchone()
            if r:
...
def add_cashout(driver_id, points_used, amount_usd, month):
    with get_connection() as conn:
        with conn.cursor() as c:
            c.execute("INSERT INTO cashouts (driver_id, points_used, amount_usd, month) VALUES (%s, %s, %s, %s)", (driver_id, points_used, amount_usd, month))
